...
(defn replay-all [game-id]
  "Replay all events returning the resulting aggregate"
  (es/apply-events m/empty-game (flatten (:transactions (es/retrieve-event-stream es/in-memory-event-store game-id)))))

(def game-id 11)

(defn play-game [] (replay-all game-id))

(defn in-game

  ([events]
   (es/clear-events es/in-memory-event-store game-id)
   (es/append-events es/in-memory-event-store game-id (es/->EventStream 0 []) events))
...
(deftest four-people-joining
  (let [cmd-enter (c/->NewPlayerEnter game-id)]
    ...
    (is (= (m/->Game nil 0) (replay-all game-id)))

    (c/handle-command cmd-enter es/in-memory-event-store)
    (is (= (m/->Game nil 1) (replay-all game-id)))

    (c/handle-command cmd-enter es/in-memory-event-store)
    (is (= (m/->Game nil 2) (replay-all game-id)))

    (c/handle-command cmd-enter es/in-memory-event-store)
    (is (= (m/->Game nil 3) (replay-all game-id)))
    ...
    (is (thrown? Exception (c/handle-command cmd-enter es/in-memory-event-store)))))
...
(deftest player-turn
  (let [wished-east-tiles [:b1 :b2 :b3 :b4 :b5 :b6 :b7 :b8 :b9 :c1 :c2 :c3 :c4]
    ...
    (let [game           (replay-all game-id)
      ...
      (is (thrown? Exception (c/handle-command (mk-discard-cmd :north) es/in-memory-event-store)))
      (is (thrown? Exception (c/handle-command (mk-discard-cmd :west) es/in-memory-event-store)))
      (is (thrown? Exception (c/handle-command (mk-discard-cmd :south) es/in-memory-event-store)))

      ;can discard only tiles owned
      (is (thrown? Exception (c/handle-command (c/->DiscardTile game-id :east :c5) es/in-memory-event-store))))

    (c/handle-command (c/->DiscardTile game-id :east :c3) es/in-memory-event-store)
    (let [game (replay-all game-id)]
    ...
    ;can't discard twice
    (is (thrown? Exception (c/handle-command (c/->DiscardTile game-id :east :b2) es/in-memory-event-store)))))
...
(deftest auction-pass
  (let [wished-east-tiles [:b1 :b2 :b3 :b4 :b5 :b6 :b7 :b8 :b9 :c1 :c2 :c3 :c4]
    ...
    (is (thrown? Exception (c/handle-command (c/->Pass game-id :east) es/in-memory-event-store)))

    ; other players can pass in any order
    (c/handle-command (c/->Pass game-id :south) es/in-memory-event-store)
    (c/handle-command (c/->Pass game-id :north) es/in-memory-event-store))

  ; but only once
  (is (thrown? Exception (c/handle-command (c/->Pass game-id :north) es/in-memory-event-store)))
...
(deftest auction-chow
  (let [wished-east-tiles  [:b1 :b2, :b3 :b4 :b5 :b6 :b7 :b8 :b9 :c1 :c2 :c3 :c4]
    ...
    (is (thrown? Exception (c/handle-command (c/->Chow game-id :east #{:b1 :b3}) es/in-memory-event-store)))

    ; only player on the right can chow
    (is (thrown? Exception (c/handle-command (c/->Chow game-id :west #{:b1 :b3}) es/in-memory-event-store)))

    ; north can Chow but only with a valid set of tiles
    (is (thrown? Exception (c/handle-command (c/->Chow game-id :north #{:b5 :b6}) es/in-memory-event-store)))
    (c/handle-command (c/->Chow game-id :north #{:b3 :b4}) es/in-memory-event-store))
...
(deftest auction-pung
  (let [wished-east-tiles  [:b1 :b2 #_____ :b3 :b4 :b5 :b6 :b7 :b8 :b9 :c1 :c2 :c3 :c4]
    ...
    (is (thrown? Exception (c/handle-command (c/->Pung game-id :east) es/in-memory-event-store)))

    ; must have appropriate tiles to pung
    (is (thrown? Exception (c/handle-command (c/->Pung game-id :north) es/in-memory-event-store)))

    ; any player can pung
    (c/handle-command (c/->Pung game-id :west) es/in-memory-event-store)
...
(deftest auction-kong
  (let [wished-east-tiles  [:b1 :b2 #_____ :b3 :b4 :b5 :b6 :b7 :b8 :b9 :c1 :c2 :c3 :c4]
    ...
    (is (thrown? Exception (c/handle-command (c/->Kong game-id :east) es/in-memory-event-store)))

    ; must have appropriate tiles
    (is (thrown? Exception (c/handle-command (c/->Kong game-id :north) es/in-memory-event-store)))

    ; not next player can kong
    (c/handle-command (c/->Kong game-id :west) es/in-memory-event-store)
...
(deftest auction-all-passed
  (let [events (conj (start-game-events m/all-tiles)
    ...
    (c/handle-command (c/->Pass game-id :north) es/in-memory-event-store)
    (c/handle-command (c/->Pass game-id :west) es/in-memory-event-store)
    (c/handle-command (c/->Pass game-id :south) es/in-memory-event-store)
...
(deftest auction-chow-pung-compete
  (let [wished-east-tiles  [:b1 :b2 :b3 :b4 :b5 :b6 :b7 :b8 :b9 :c1 :c2 :c3 :c4]
    ...
    (c/handle-command (c/->Chow game-id :north #{:b4 :b5}) es/in-memory-event-store)
    (c/handle-command (c/->Pung game-id :west) es/in-memory-event-store)
    (c/handle-command (c/->Pass game-id :south) es/in-memory-event-store)
...
(comment deftest auction-chow-hule-compete
         (let [wished-east-tiles  [:b3 :s1 :s2 :s3 :s4 :s5 :s6 :s7 :s8 :s9 :s1 :s2 :s3]
           ...
           (handle-command (->Chow game-id :north #{:b4 :b5}) es/in-memory-event-store)
           (handle-command (->Pass game-id :south) es/in-memory-event-store)
           (handle-command (->Hule game-id :west) es/in-memory-event-store)
